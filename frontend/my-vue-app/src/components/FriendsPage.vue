<template>
  <div class="friends-page" style="padding: 20px">
    ...
    <div
      v-else-if="searchTerm && filteredFriendList.length === 0"
      class="no-results"
    >
...
const filteredFriendList = computed(() => {
  ...
  return friends.filter((friend) => {
    const nameMatch =
      friend.name && String(friend.name).toLowerCase().includes(term);
    const idMatch =
      friend.accountId && String(friend.accountId).toLowerCase().includes(term);
